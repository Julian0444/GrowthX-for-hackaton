...
## Session — 2026-09-09 05:15 UTC

### Resumen de la sesión

Se implementó el ticket **14 — Reabrir la decisión exacta desde el dashboard** (`.scratch/evaluacion-persistida/issues/14-reabrir-desde-dashboard.md`), autorizado explícitamente por Julian (cubrió solo este ticket; su bloqueo por 13 ya estaba resuelto). El ticket quedó en `Status: ready-for-human` con los 8 criterios marcados y `## Comments` completa (qué se hizo, tests con salida real, desvíos autorizados, límites). **No se hizo commit ni push**: los hace Julian. Nota: al inicio de esta sesión el slice 01–13 ya estaba commiteado (`be3fcac`); el árbol sucio actual es solo el ticket 14.

Alcance ampliado autorizado en sesión (AskUserQuestion, opción «Todo»): la lista de archivos del ticket era pre-10/12 (`atlas-shell`, `result-rail`, `opportunity-drawer`), así que se autorizó tocar `components/research-dashboard/research-dashboard.tsx` y `comparison-panel.tsx` (el dashboard y el «panel actual» desde 10–13), `research-types.ts`, `lib/server/evaluations/dashboard-store.ts`, `evaluations/wire.ts` + `service.ts` (vínculo de la reevaluación), `package.json` (el e2e nuevo entra en `pnpm test`) y `app/api/decisions/route.ts` (400 por `snapshotId` malformado).

Qué se construyó:

- **Lista mínima de evaluaciones guardadas** en la sección Decisiones del dashboard (componente nuevo `components/research-dashboard/evaluation-list.tsx`; no es un producto de historial aparte). `GET /api/evaluations` suma `evaluations` (runs `investment_comparison` con estado, fechas de creación/evaluación, perfil —producto, versión, presupuesto, objetivo—, ediciones con nombre, snapshot y la ÚLTIMA revisión de cada decisión con fecha original, condiciones abiertas y campaña), `evaluationProfiles` y `evaluationFilter` (`dashboard-store.ts › readSavedEvaluations`). Filtro EXPLÍCITO por identidad de perfil `?profileId=` (uuid; malformado → 400), nunca por texto de producto. Entrar a Decisiones relee la lista.
- **Enlace interno y selección recuperable**: la URL lleva `?run=&decision=&view=campaign` (`evaluationHref` / `parseEvaluationFocus` en `evaluation-list.tsx`); el dashboard guarda el foco y lo escribe con `replaceState`; el panel enfoca la decisión (`data-decision-id`, `scrollIntoView`) o abre directamente la campaña. `CampaignDraftPanel` recibe `meta` (run, snapshot, revisión, fecha, enlace interno). Desde el panel se abren el dossier de la edición y el expediente del organizador; «Volver a …» restaura la misma decisión. El mapa no interviene.
- **Lectura sin recálculo**: reabrir = `GET /api/evaluations/:id` (`runs.result` persistido) + `GET /api/decisions?snapshotId=` (+ `GET` del run anterior en reevaluaciones). Sin proveedores, fixtures, pipeline mundial ni scores nuevos.
- **Fecha/revisión original y vigencia actual**: `projectComparisonResult(result, { readAt })` (`opportunity-adapter.ts`) suma por candidato `organizerId`, `fixedRevisions` (revisiones de edición/organizador y claims fijados por el snapshot) y `currentValidity` (aviso si el inicio declarado ya pasó respecto de la lectura y NO había vencido al evaluar; no altera estado, condiciones ni score). El panel muestra «obtenido <fecha>», la línea de revisiones fijadas, «revisión N · registrada el <decidedAt>» y el historial de revisiones conservadas.
- **Reevaluar** (acción explícita del panel): `startComparison` con `previousRunId` (nuevo en `ComparisonStartBody`: parseo estricto uuid, en el hash del payload; `acceptComparison` valida bajo RLS que sea una comparación del tenant —ajena/inexistente → 400— y lo persiste en `runs.input`). Mismo perfil (`profileRunId` = el run actual) y mismo conjunto; el worker evalúa con las revisiones vigentes. El nuevo run muestra «Reevaluación de …», el snapshot anterior y, por candidato, la «Decisión previa» con enlace. Nada anterior se edita.
- **Errores distinguibles**: `GET/PATCH /api/decisions/:id` y `GET /api/decisions?snapshotId=` devuelven 400 ante ids malformados (antes 500 por cast de uuid). El panel distingue «leyendo» (no ofrece «Registrar decisión» hasta terminar), «no disponible» (con reintento), «sin sesión», «decisión enlazada inexistente» y «sin decisión». La Launch Room solo sobrevive como TIPOS en el contrato v0 congelado (`growxth.ts › L5`), sin consumidores.
- **Cosmético**: la caja de progreso dice «Evaluación» para runs de comparación y «Abrir evaluación anterior (esta es una reevaluación)».
- **Test nuevo `tests/e2e/reopen-evaluation.spec.ts`** (Next, worker y PostgreSQL REALES; catálogo sintético = fixture de 09 con fechas relativas + una edición de SF con costo cotizado USD 1500; sin claves de proveedor en el entorno): (1) guardar elección condicional con condición completa → campaña con su enlace interno → descarte de otra alternativa → cerrar contexto, apagar worker y **apagar Next**; (2) **reiniciar Next**, contexto nuevo, mismos ids/motivos/claims/campaña; cero llamadas (contador del navegador que aborta proveedores/pipeline/importador y cualquier POST/PATCH en fase de lectura + invariantes en PostgreSQL + worker apagado); 400/404/401 distinguibles y tenant señuelo; (3) lista → decisión → expediente → dossier → volver → campaña, sin mapa; (4) vigencia actual con `context.clock.setFixedTime` en un contexto aislado, estado histórico intacto; (5) reevaluar: `previousRunId` inválido → 400, run vinculado con otro snapshot, decisión previa `deepEqual`, navegación a ambas versiones; (6) segundo presupuesto (perfil v2, 500 USD): la edición cotizada queda excluida en el snapshot nuevo y condicionada en el anterior, filtro por perfil 2/1, `GET ?profileId=` coherente. Guarda en las tres comparaciones: redacción `deterministic_only` con «GEMINI_API_KEY ausente» — si un worker ajeno con claves procesa los runs, el test falla con mensaje explícito.

Verificación: una corrida completa de `pnpm --dir frontend test` en verde (**159 + 11 + 7**, exit 0), `pnpm --dir frontend lint` exit 0, `npx tsc --noEmit` limpio, `pnpm build` OK (rutas presentes), `test:baseline` 8/8 sin INESPERADO. Dos corridas completas posteriores (tras dos retoques cosméticos) fallaron en el **e2e del ticket 10** — ver bloqueos — y el e2e de 14 corrido solo volvió a pasar 7/7 en ese mismo momento. Capturas revisadas (`reopen-list`, `reopen-decision`).

Decisiones y hallazgos no obvios:

- **Reloj de Playwright es por contexto**, no por página: el subtest de vigencia usa un `browser.newContext()` propio para no contaminar la sesión principal (la primera versión dejó `Date` fijado en 2029 en los subtests siguientes; se vio en las capturas).
- **`assert.equal(status, 202, await r.text())` consume el body** antes de `r.json()` (bug de helper de la primera corrida; ahora se lee `text()` una vez y se parsea).
- **`readAt` es el reloj del navegador**: el aviso de vigencia es una proyección; el resultado histórico viene del servidor. Si se prefiere el instante del servidor, es un campo nuevo de `EvaluationRunView` (no se tocó).
- **`DecisionRead` no trae `runId`** (no se tocó `decisions/store.ts`): el enlace interno siempre lleva `run=`; un `?decision=` sin `run=` no se resuelve.
- `next-env.d.ts` lo modifica `pnpm build` (artefacto); se restauró con `git checkout` cada vez.

### Estado actual

- Rama: `main`. HEAD: `be3fcac` (slice 01–13 commiteado por Julian antes de esta sesión).
- Cambios de esta sesión, sin commitear (16 rutas): `M frontend/app/api/decisions/[id]/route.ts`, `M frontend/app/api/decisions/route.ts`, `M frontend/app/api/evaluations/route.ts`, `M frontend/components/atlas/campaign-panel.tsx`, `M frontend/components/research-dashboard/comparison-panel.tsx`, `M frontend/components/research-dashboard/research-dashboard.tsx`, `M frontend/components/research-dashboard/research-types.ts`, `M frontend/lib/api/atlas-client.ts`, `M frontend/lib/api/opportunity-adapter.ts`, `M frontend/lib/server/evaluations/dashboard-store.ts`, `M frontend/lib/server/evaluations/service.ts`, `M frontend/lib/server/evaluations/wire.ts`, `M frontend/package.json`; nuevos `frontend/components/research-dashboard/evaluation-list.tsx`, `frontend/tests/e2e/reopen-evaluation.spec.ts`; ticket 14 (Status, criterios, Comments). Sin migración nueva; contratos de 07 intactos.
- Tests: `pnpm --dir frontend test` → 159 + 11 + 7 en verde en la corrida completa registrada; en las dos últimas corridas completas, 159 verde y el e2e de 10 en rojo por el worker ajeno (abajo); e2e de 14 solo → 7/7. Lint/tsc/build/baseline en verde.
- Infra local: contenedor `growthx-postgres` arriba, migraciones 001–006 aplicadas (nada nuevo). El e2e de 14 limpia sus tenants `reopen-e2e-%`, cancela sus jobs y no deja directorios temporales. **Procesos ajenos a esta sesión que siguen vivos y no se tocaron:** `next dev --webpack` del workspace (PID 88154, ~1 h 12 min al cierre) y un worker de desarrollo `node --env-file=.env.local worker/index.ts` (PID 99139, ~53 min, con claves reales de `.env.local`).
- Tickets: **01–14 en `ready-for-human`**; 15 en `needs-triage`, no autorizado.

### Tareas pendientes (en orden)

1. **Julian**: revisar los Comments de 14 (incluyen el alcance ampliado, la explicación de las corridas en rojo del e2e de 10 y los límites: `readAt` del navegador, `DecisionRead` sin `runId`, lista solo de comparaciones, reevaluar con el mismo perfil) y commitear el ticket 14 (mensaje en español).
2. **Antes de correr `pnpm --dir frontend test`**: apagar el worker de desarrollo (PID 99139 o el que esté escuchando la cola con `.env.local`). Con él vivo, el e2e de 10 falla en su subtest 1 («el run sigue `queued`») y arrastra cinco subtests; el e2e de 14 pasa igual pero detectaría a ese worker si procesa sus runs.
3. **Ticket 15** (último del slice: aceptación de caídas y aislamiento): requiere autorización explícita y pase a `ready-for-agent`. 14 le deja listos el enlace interno autenticado, la lista por identidad/perfil, la reevaluación con vínculo y el e2e con reinicio real de Next como patrón reutilizable (harness inline: `startNext()`, `armContext()`, `counts()`).
4. **Decisiones abiertas D1–D5**: sin cambios. D3 (acceso del usuario real al reabrir) sigue resuelta solo para tests (sesiones insertadas en `growthx.sessions`).
5. Sigue abierta la decisión de promover el alias `@/` a un preload compartido; 14 lo esquivó igual que 02–13 (imports relativos `.ts`; el spec no importa `.tsx`, compone los `href` a mano).

### Bloqueos y advertencias

- **Worker ajeno en la cola compartida**: los e2e (10 y 14) y los tests de integración comparten la cola pg-boss `evaluation-run` con cualquier worker que escuche en esta máquina. El e2e de 10 asume que no hay otro worker (aserción `queued` antes de arrancar el suyo) — no se tocó ese spec (fuera de alcance); robustecerlo sería un ticket propio. Un worker con claves reales además puede llamar a Gemini con los runs de test. Fix: apagar el worker de desarrollo mientras corren los tests (anotado en la memoria del proyecto `infra-local-ticket-08`).
- **Autorización por ticket**: 15 necesita visto bueno explícito de Julian y `ready-for-agent`. En esta sesión el criterio fue alcance estricto con UNA pregunta previa (AskUserQuestion con el conjunto completo de archivos extra) — confirmar el criterio por ticket.
- **Append-only e inmutabilidad por grants** (12/13) siguen vigentes: reevaluar es OTRO run con OTRO snapshot; una decisión se corrige con PATCH (revisión nueva). No «arreglar» un permission denied con grants.
- **Aserciones destinadas a caducar**: el e2e de 14 asevera textos de UI del panel y de la lista («Elegida · elección condicional», «Reevaluar (nuevo run vinculado)», «Volver a la comparación», labels de la lista) y `data-testid` nuevos (`saved-evaluation`, `saved-decision`, `decision-link`, `campaign-draft-meta`, `current-validity-notice`, `reevaluation-note`, `previous-decision`, `reevaluate`); quien cambie esos textos debe actualizarlas.
- Siguen vigentes: drift esperado del manifiesto v0 (baseline en modo diagnóstico; no regenerar `characterization.json`/`manifest.json` — la sombra v0 lo LEE), higiene de la cola pg-boss ante tests interrumpidos, timers de 4,5 s (`LIVE_SIGNAL_WAIT_MS`), warnings `MODULE_TYPELESS_PACKAGE_JSON` preexistentes, `next-env.d.ts` reescrito por `pnpm build` (restaurar con `git checkout`), y la memoria del proyecto («en prompts para sesiones-worker, no repetir lo que ya dice el ticket .md»; `infra-local-ticket-08` actualizada con la nota del worker ajeno).

### Primera acción sugerida para la próxima sesión

Leer los Comments de `.scratch/evaluacion-persistida/issues/14-reabrir-desde-dashboard.md`, verificar `docker ps | grep growthx-postgres` (si no está: `pnpm --dir frontend db:up && pnpm --dir frontend db:migrate`), comprobar con `pgrep -fl "worker/index.ts"` que no haya un worker de desarrollo escuchando la cola (apagarlo si lo hay), correr `pnpm --dir frontend test` para confirmar 159 + 11 + 7 en verde, y esperar las decisiones de Julian (commit de 14; autorización de 15 y D1–D5) antes de escribir código. Para ver la demo de reapertura: abrir un run de comparación con decisión (o crear uno: Eventos → comparar → «Registrar decisión»), copiar el «Enlace interno» de la decisión, cerrar la pestaña, reiniciar `next dev` y pegar el enlace: misma decisión, misma campaña; en Decisiones está la lista con el filtro por perfil y «Reevaluar» en el panel.

---

## Session — 2026-09-09 06:05 UTC

### Resumen de la sesión

Se implementó el ticket **15 — Demostrar el slice completo bajo caídas y cruces de tenant** (`.scratch/evaluacion-persistida/issues/15-aceptacion-caidas-y-aislamiento.md`), el ÚLTIMO del slice, autorizado explícitamente por Julian (cubrió solo este ticket). Quedó en `Status: ready-for-human` con 9 de 10 criterios marcados y `## Comments` completa (qué se hizo, tests con salida real, desvíos, límites). El criterio de **aceptación humana con eventos reales quedó SIN marcar a propósito**: lo bloquean D3/D4, como declara el propio ticket. **No se hizo commit ni push**: los hace Julian.

Qué se construyó:

- **`frontend/tests/integration/worker-recovery.test.ts`** (nuevo, 10 subtests): matriz de caídas con PostgreSQL, pg-boss y el worker como PROCESO separado. SIGKILL real en los cuatro puntos del criterio (obtención EN VUELO —barrera por archivo marker del transporte, sin sleeps—, tras guardar claims, tras confirmar el snapshot, antes de confirmar el job) más el corte entre commit de paso y siguiente trabajo; en cada punto el MISMO run llega a terminal sin perder avances ni duplicar efectos. Además: cola caída en la aceptación (cero filas/jobs; reintento del cliente trazado), entrega duplicada + dos workers vivos, carrera determinística de dos entregas del mismo run (compuertas), timeout/salida inválida del modelo (degradación con `usage: null` explícito), fuente que agota 4 intentos (run `failed` visible, `result: null`), y reconstrucción run→steps→claims→snapshot→decisión con auditoría de secretos y de texto privado del HTML.
- **`frontend/tests/integration/tenant-isolation.test.ts`** (nuevo, 8 subtests): con IDs CONOCIDOS del otro tenant — API (404 idéntico a inexistente), listados, dossiers, PATCH, comparación/importación/decisión con referencias ajenas, idempotencia (clave ajena crea run PROPIO), sesiones (expirada 401; sin pertenencia ni puede existir, por FK), pool `max: 1` (mismo `pg_backend_pid()`; transacción fallida a mitad no filtra contexto), roles sin `rolsuper`/`rolbypassrls`, RLS ante UPDATE/INSERT ajenos, y jobs forjados (tenant discordante / forma inválida) rechazados por el worker REAL sin tocar el run. Ambos catálogos llevan centinelas y TODAS las respuestas de cruce se auditan contra ellos.
- **`frontend/tests/e2e/persisted-evaluation.spec.ts`** (nuevo, 4 subtests; entra en `pnpm test` y `test:e2e` vía `package.json`): la Demostración del ticket con navegador/Next/worker reales — pegar URL de Luma (transporte de fixture) → cerrar pestaña → worker cae por SIGKILL → reinicio → dossier con fecha/acceso/costo PENDIENTES → comparar (importada `conditional`) → elegir condicionalmente → cerrar TODO → reiniciar Next → recuperar por enlace interno (identidad/evidencia/condiciones cotejadas, cero llamadas, cero escrituras) → señuelo sin leer ni PATCH; más perfil → organizadores → antecedente Berlín con empresa/rol («Resultado comercial desconocido») → evento → decisión, con `sf-map` en 0 y `ed-berlin-future` fuera de la lista local.
- **Barreras de prueba en runtime** (env-gated, inertes sin las variables): `lib/server/evaluations/run-worker.ts` expone `testBarrier` (`before_step_commit:<paso>` / `after_step_commit:<paso>` / `before_job_ack`); `worker/index.ts` las cablea desde `GROWTHX_WORKER_KILL_AT` (SIGKILL a sí mismo) y suma `GROWTHX_WORKER_LUMA_FIXTURE`/`GROWTHX_WORKER_LUMA_CALLS_FILE` (transporte de obtención controlado por archivo; toda la validación del adaptador sigue corriendo).
- **Único cambio de comportamiento de runtime, demostrado en ROJO por la matriz y corregido**: el path de fallo de `processEvaluationRun` degradaba lo confirmado por una entrega competidora (paso completado → `failed`; run `completed` → `queued`/`failed`). Las dos actualizaciones ahora llevan guarda (`state <> 'completed'` / `not in ('completed','failed')`). Rojo/verde verificado.
- **Nota de evidencia** `.scratch/evaluacion-persistida/aceptacion-15.md` (nueva): matriz criterio→prueba, salidas reales, guion de demo manual de recuperación/aislamiento, y el apartado de aceptación humana PENDIENTE con campos fecha/revisor sin completar (nada se declara catálogo real por pasar con fixtures).

Decisiones de diseño no obvias:

- **Bases dedicadas por suite** (`growthx_t15_recovery` / `growthx_t15_isolation`, recreadas con `drop database … with (force)` al inicio): `node --test` corre archivos en paralelo y la cola pg-boss es POR BASE — sin esto el worker de `evaluation-run.test.ts` levantaría los jobs de la matriz sin transporte controlado (red real), y los workers con barreras de kill matarían jobs ajenos. El lock advisory de `runMigrations` es por base, así que la carrera de `ALTER ROLE` entre suites se reintenta en el test (hasta 5 veces ante «tuple concurrently updated»); `migrate.ts` NO se tocó.
- **Re-entrega acelerada tras SIGKILL**: un job `active` de un worker muerto expira recién a los 120 s; los tests lo reponen a `retry` con `start_after = now()` vía SQL admin — lo mismo que hace el mantenimiento de la cola, sin esperar. El estado de negocio no se toca.
- **La carrera no puede forzar el unique del snapshot desde afuera** (el chequeo de existencia de `persistOfficialSnapshot` vive dentro de su transacción; una entrega tardía converge a `already_persisted`): el fallo real demostrable era el del path de fallo, y eso es lo que se corrigió. La unicidad queda demostrada por el unique `(tenant, run)` + los subtests de re-entrega/duplicado.
- Alcance: todo lo tocado está dentro de los módulos que el ticket nombra (`frontend/worker/`, `run-worker.ts`, «scripts de pruebas» = `package.json`, los tres tests y la nota). `lib/server/db/`, `lib/server/auth/` y las rutas quedaron SIN cambios: la matriz no demostró fallos ahí.

### Estado actual

- Rama: `main`. HEAD: `be3fcac` (slice 01–13 commiteado; 14 y 15 sin commitear).
- Cambios sin commitear: los 16 archivos del ticket 14 (ver sesión anterior) MÁS los de esta sesión: `M frontend/lib/server/evaluations/run-worker.ts`, `M frontend/worker/index.ts`, `M frontend/package.json` (spec nuevo en `test`/`test:e2e`), `M` ticket 15, y nuevos `frontend/tests/integration/worker-recovery.test.ts`, `frontend/tests/integration/tenant-isolation.test.ts`, `frontend/tests/e2e/persisted-evaluation.spec.ts`, `.scratch/evaluacion-persistida/aceptacion-15.md`.
- Tests: `pnpm --dir frontend test` → **179 + 11 + 7 + 5 = 202 pass / 0 fail / 0 skipped, exit 0** (TRES corridas completas consecutivas idénticas). `pnpm --dir frontend lint` → exit 0 sin warnings. `npx tsc --noEmit` limpio. `pnpm build` OK (`next-env.d.ts` restaurado con `git checkout`). `test:baseline` → 8/8, O1–O5 «corregido», sin INESPERADO.
- Infra local: contenedor `growthx-postgres` arriba (migraciones 001–006, nada nuevo). Quedan dos bases SOLO de test `growthx_t15_recovery`/`growthx_t15_isolation` (se recrean solas en cada corrida; borrables). **El worker de desarrollo de Julian (PID 99139, con claves de `.env.local`) fue apagado esta sesión** — el handoff de 14 documenta que rompe los e2e; relanzar con `pnpm --dir frontend worker` cuando haga falta. El `next dev` del workspace (PID 88154) sigue vivo, no se tocó. Memoria del proyecto `infra-local-ticket-08` actualizada.
- Tickets: **01–15 en `ready-for-human` — el slice completo está implementado**. Solo queda abierto el criterio de aceptación humana del 15 (bloqueado por D3/D4).

### Tareas pendientes (en orden)

1. **Julian**: revisar los Comments de 14 y 15 (el de 15 incluye el fix del path de fallo, las barreras env-gated, las bases dedicadas y el criterio sin marcar) y commitear todo (mensaje en español).
2. **Decisiones abiertas D3/D4**: cerrarlas habilita la aceptación humana del 15 — completar el apartado final de `.scratch/evaluacion-persistida/aceptacion-15.md` (2–5 eventos reales de SF, dos organizadores con antecedentes, URL de Luma permitida, fecha/revisor) y marcar el criterio pendiente del ticket.
3. **D1/D2/D5**: siguen abiertas — la demo se presenta como evaluación factual/condicional con objetivo provisional, sin ranking comercial (D2: `approvedPolicyFor` devuelve null) y sin afirmar operación compartida (todo local).
4. Sigue abierta la decisión de promover el alias `@/` a un preload compartido; 15 lo esquivó igual que 02–14 (imports relativos `.ts`).

### Bloqueos y advertencias

- **Aceptación humana del 15**: NO declarar el slice «aceptado» por pasar con fixtures; el criterio queda sin marcar hasta D3/D4, con el guion y los campos listos en `aceptacion-15.md`.
- **Hooks de test del worker**: `GROWTHX_WORKER_KILL_AT` y `GROWTHX_WORKER_LUMA_FIXTURE` son SOLO para tests/demo (env-gated; en producción no se setean). No usarlos para «arreglar» comportamiento.
- **Guarda del path de fallo**: un intento perdedor ya no degrada un run terminal ni un paso completado; si algún test futuro espera que un fallo tardío pise el estado, el comportamiento correcto es el nuevo.
- **Bases `growthx_t15_*`**: si un test de integración nuevo necesita worker propio + cola, seguir el mismo patrón (base dedicada recreada + retry de migraciones por la carrera de `ALTER ROLE` entre bases).
- **Worker de desarrollo**: apagado durante los tests SIEMPRE (con él vivo, el e2e de 10 falla y los e2e de 14/15 detectan al «worker ajeno con claves» y fallan explícitamente con ese mensaje).
- Siguen vigentes: drift esperado del manifiesto v0 (baseline en modo diagnóstico; no regenerar `characterization.json`/`manifest.json` — la sombra v0 lo LEE), higiene de la cola pg-boss ante tests interrumpidos, timers de 4,5 s (`LIVE_SIGNAL_WAIT_MS`), warnings `MODULE_TYPELESS_PACKAGE_JSON` preexistentes, `next-env.d.ts` reescrito por `pnpm build` (restaurar con `git checkout`), y la memoria del proyecto («en prompts para sesiones-worker, no repetir lo que ya dice el ticket .md»; `infra-local-ticket-08` actualizada con las bases t15 y los hooks del worker).

### Primera acción sugerida para la próxima sesión

Leer los Comments de `.scratch/evaluacion-persistida/issues/15-aceptacion-caidas-y-aislamiento.md` y la nota `.scratch/evaluacion-persistida/aceptacion-15.md`, verificar `docker ps | grep growthx-postgres` y `pgrep -fl "worker/index.ts"` (apagar cualquier worker de desarrollo), correr `pnpm --dir frontend test` para confirmar 179 + 11 + 7 + 5 (202 en total) en verde, y esperar las decisiones de Julian (commit de 14+15; D3/D4 para la aceptación humana) antes de escribir código.
