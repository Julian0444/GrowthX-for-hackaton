...
import { projectComparisonResult, projectEditionDossierView, snapshotEditionDossiers } from "../../lib/api/opportunity-adapter"
...
const STATES: Record<string, string> = { queued: 'En cola', running: 'En ejecución', completed: 'Completado', failed: 'Fallido', pending: 'Pendiente', partial: 'Parcial · lectura pendiente', insufficient: 'Cobertura insuficiente' }
const STEPS: Record<string, string> = { validate_profile: 'Validar perfil', discover_sources: 'Buscar y conservar fuentes con Exa', research_catalog: 'Investigar catálogo SF', fetch_event_page: 'Obtener página del evento', persist_dossier: 'Persistir dossier', read_relationship_sources: 'Leer antecedentes y proyectos', persist_relationships: 'Guardar relaciones y evidencia', evaluate_candidates: 'Evaluar candidatos y confirmar snapshot', compose_narrative: 'Redacción opcional del modelo', publish_result: 'Publicar resultado' }
...
export function ResearchDashboard() {
  ...
  const editions = savedComparison ? snapshotEditionDossiers(savedComparison.bundle) : isDiscovery ? [] : result?.editions ?? catalog ?? []
    ...
    stop.current = pollEvaluation(id, outcome => {
      ...
      if (outcome.status === 'ok') {
        ...
        if (restoreSection) {
          ...
          if (detailSequence.current === detailSeq) setSection(outcome.run.workflowVersion === 'investment-comparison/1' ? 'Decisiones' : [LUMA_WORKFLOW, BACKGROUND_WORKFLOW].includes(outcome.run.workflowVersion) ? 'Eventos' : 'Organizadores')
        }
        setRun(current => {
  ...
  async function openEdition(id: string) {
    ...
    if (savedComparison) { setNote('La edición no está incluida en esta comparación guardada.'); return }
    setDetailBusy(true)
  ...
  async function compare() {
    ...
    const profileRunId = (run && ['sf-organizers/1', DISCOVERY_WORKFLOW].includes(run.workflowVersion) ? run.runId : null) ?? home?.runs[0]?.runId ?? null
    if (!profileRunId) {
  ...
  return <div className="atlas research-dashboard">
    <aside className="research-sidebar"><button className="research-brand" onClick={() => navigate('Resumen')}><span className="brand-title">Growth Atlas</span><span>Investigación · SF</span></button>
      ...
      <p>Investigación acotada.<br/>Fuentes antes de decidir.</p>
    </aside>
    <div className="research-main" ref={content}><div className="research-content">
      ...
      {!isDiscovery && (coverage ? <div className="research-coverage" data-testid="research-coverage"><span className="coverage-count"><b>{coverage.organizers}</b> organizadores · <b>{coverage.editions}</b> ediciones en el catálogo</span><span>Carga / obtención: {coverage.verifiedAt.join(' · ') || 'sin cargas registradas'}</span><span>{coverage.material.includes('synthetic') ? 'Material sintético: prueba el mecanismo; no acredita cobertura real de SF.' : coverage.material.includes('imported') ? 'Incluye material obtenido automáticamente, sin curación humana.' : coverage.material.length ? 'Material curado' : 'Sin catálogo cargado'}</span>{result && <span>Investigación guardada al {result.evaluatedAt}</span>}</div> : !note && <p role="status">Leyendo cobertura…</p>)}
      {runId && <section className={`research-progress progress-${run?.state ?? "pending"}`} aria-label="Estado persistido" data-testid="run-progress"><p>{run?.workflowVersion === 'investment-comparison/1' ? 'Evaluación' : 'Investigación'} <span className="research-meta">{runId}</span> · {run ? STATES[run.discovery?.progress.status ?? run.state] : 'Consultando estado…'}</p>
      ...
      {run?.researchPlan && section === 'Organizadores' && !detail && <details data-testid="saved-brief-questions"><summary>Preguntas guardadas · brief v{run.profile.profileVersion}</summary><ol>{run.researchPlan.questions.map(q => <li key={q.id}>{q.text}</li>)}</ol></details>}
      ...
      {detail ? <div className="research-detail"><button className="research-button" onClick={() => { detailSequence.current++; setDetail(null) }}>Volver a {section.toLowerCase()}</button>
        <p className="research-meta">{result || savedComparison ? 'Fuentes y revisiones conservadas en la investigación' : 'Lectura actual del catálogo persistido'}</p>
        ...
        {section === 'Organizadores' && isDiscovery && run?.discovery && <DiscoveryPanel discovery={run.discovery} onResearch={id => openRun(id)} />}
        {section === 'Organizadores' && !isDiscovery && <section className="organizers-section"><h2>Coincidencias con antecedentes</h2>
        ...
        {section === 'Eventos' && <section className="events-section"><div className="research-actions"><h2>Ediciones de San Francisco</h2><div role="group" aria-label="Vista de eventos"><button className="research-button" aria-pressed={!mapOpen} onClick={() => setMapOpen(false)}>Lista</button><button className="research-button" aria-pressed={mapOpen} onClick={() => setMapOpen(true)}>Mapa</button></div></div>
          ...
          {mapOpen && <SfEventMap selectedEditionId={presentation.selectedEditionId} editions={sfEditions} onEdition={id => void openEdition(id)} />}
          ...
          <div className="edition-grid" data-testid="sf-edition-list">{sfEditions.map(read => { const e = latestEdition(read); const view = projectEditionDossierView(read); return <article className="research-event" key={read.editionId} data-edition-id={read.editionId} data-edition-revision-id={e.id} data-selected={presentation.selectedEditionId === e.editionId} data-source-ids={read.sources.map(s => s.id).sort().join(',')}>
            ...
            <p data-testid="event-location-summary">{view.publicLocation.originalAddress ?? view.publicLocation.venue ?? view.publicLocation.city} · {view.withoutPointReason ?? (view.publicLocation.resolution?.accuracy === "interpolated" ? "Dirección aproximada (interpolada)" : `Ubicación ${view.publicLocation.status}`)}</p>
