"""Source-identical copy and isolated PostgreSQL 55460; never load provider keys."""
import hashlib, json, os, shutil, subprocess, sys, tempfile, time
from pathlib import Path
OUT=Path(__file__).resolve().parent
ROOT=OUT.parents[2]
if sys.argv[1] in ['prepare','prepare-webpack']:
    work=Path(tempfile.mkdtemp(prefix='growthx-dp10-'));copy=work/'frontend'
    ...
    for ticket in ['DP-05','DP-06','DP-07','DP-08','DP-09','DP-10']:(work/'DemoPuentes/evidence'/ticket).mkdir(parents=True,exist_ok=True)
...
env.update({'NEXT_TELEMETRY_DISABLED':'1','GROWTHX_ADMIN_DATABASE_URL':'postgres://growthx:growthx@127.0.0.1:55460/growthx','GROWTHX_DATABASE_URL':'postgres://growthx_app:growthx_app_dev@127.0.0.1:55460/growthx','GROWTHX_WORKER_DATABASE_URL':'postgres://growthx_worker:growthx_worker_dev@127.0.0.1:55460/growthx','GROWTHX_QUEUE_DATABASE_URL':'postgres://growthx_queue:growthx_queue_dev@127.0.0.1:55460/growthx','GROWTHX_E2E_SCREENSHOT_DIR':str(OUT/'screenshots'),'DP04_PRODUCTION_DIR':str(copy),'DP05_PRODUCTION_DIR':str(copy),'DP06_PRODUCTION_DIR':str(copy),'DP09_EVIDENCE_DIR':str(OUT),'DP10_EVIDENCE_DIR':str(OUT)})
name=sys.argv[1];command=sys.argv[2:];start=time.monotonic()
if name.startswith('real'):
    for key in ['GROWTHX_ADMIN_DATABASE_URL','GROWTHX_DATABASE_URL','GROWTHX_WORKER_DATABASE_URL','GROWTHX_QUEUE_DATABASE_URL']:env[key]=env[key].rsplit('/',1)[0]+'/growthx_dp10_real'

with (OUT/f'{name}.log').open('w') as log: result=subprocess.run(command,cwd=copy,env=env,stdout=log,stderr=subprocess.STDOUT)
report={'command':command,'cwd':str(copy),'databasePort':55460,'databaseName':'growthx_dp10_real' if name.startswith('real') else 'growthx','providerKeysLoaded':False,'exitCode':result.returncode,'durationSeconds':round(time.monotonic()-start,2)}
