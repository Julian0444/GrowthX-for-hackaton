...
## Corte inicial de soporte del caso (registro histórico)
...
**Decisión: mantener el comprador ilustrativo.** Cobertura: tres opciones futuras SF, antecedente positivo de proyectos de AI Tinkerers, historial de Vultr en un rol pasado de sponsor, ausencia explícita de ediciones previas de Hackathons.team y una ubicación pública completa con coordenadas enlazadas. El conteo corregido es **dos organizadores actuales con antecedentes, uno con historia como organizador**: AIT organizando en SF y Vultr patrocinando en París. No se afirma que Vultr organizara RAISE ni que Hackathons.team tenga resultados anteriores. La exigencia añadida en una revisión intermedia de dos historiales locales era incorrecta: el ticket restringe a SF las oportunidades, no todos los antecedentes. La cobertura permite comparar sin igualar los roles.
...
| Sponsors/proyectos/recaps de edición; positivo y restricción | Cumple el mínimo positivo/restricción: AIT diciembre 2025 con proyectos y roles. Dos entidades hoy organizadoras con antecedentes, uno de ellos como sponsor internacional; solo AIT aporta historia como organizador. Restricciones de acceso, costos, historial y vigencia de la fuente de O2. | [ANT-01–05](dp-01-antecedentes.md), F4/F5. |
| Hallazgo que cambia investigación/modalidad/condición | Cumple: antes/después ilustrativo explícito, de actividad técnica genérica a integración con repo/demo; en la iniciativa sin resultados propios se exigen entregables y evidencia consentida. Se detallan condiciones y qué ocurre si no se obtienen. Se retira la prioridad global de O3: el mayor plazo no resuelve sus vacíos de organización, sede y condiciones. | [Hallazgo](dp-01-caso-y-fuentes.md#hallazgo-que-cambia-la-decisión). |
...
| Conjunto esperado, fragmentos y afirmaciones prohibidas; fixtures separados | Cumple: R1–R15 vinculados a A01–A15, B01–B28 y C01–C08; soporte por atributo/edición, fecha, estado y pasaje específico. Premio, herramienta, rol y eficacia separados; material real e ilustrativo separado del fixture. | [Referencia](dp-01-caso-y-fuentes.md#conjunto-de-referencia-revisado) y [soporte por afirmación](dp-01-afirmaciones.md). |
...
6. **Revisión local inicial de entrega (antes de la corrección):** 37 enlaces Markdown locales resueltos, siete casillas de aceptación comprobadas contra esta matriz, cuatro fechas futuras dentro de ventana y dos conversiones UTC/PDT correctas. `git diff --check` sin errores; comprobación adicional de los seis Markdown de esta entrega, incluidos los no rastreados. Huellas iniciales comparadas: tres archivos previos modificados por DP-01; cambios concurrentes documentados aparte.
...
## Revisión de inconsistencias y contraste con Claude

Revisión posterior del 10/09/2026:

1. **Organizadores:** se conservan las fichas con identidad, responsables conocidos, trayectoria y audiencia declarada. Se corrige la etiqueta intermedia de cobertura local insuficiente: hay dos organizadores actuales con antecedentes, AIT como organizador y Vultr como sponsor, y uno con historia en el rol de organizador. París es un antecedente válido, excluido de oportunidades/mapa SF. La diferencia de rol ya estaba registrada; no corresponde inventar una restricción geográfica.
2. **Consecuencia para la decisión:** ahora figura un antes/después inequívoco, expresamente ilustrativo, con condiciones de avance y de descarte de modalidad. El brief ya descartaba comprar solo logo. El cambio va de una actividad técnica genérica a integración con entregables; no inventa una compra anterior ni confirma disponibilidad.
3. **Soporte específico:** reabiertas galería y fichas AIT. Premio, herramienta declarada y función técnica tienen pasajes separados. A01–A15 documentan esa relación; B01–B28 individualizan los datos decisivos de oportunidades y C01–C08 los roles/identidades. Cada R1–R15 apunta a sus afirmaciones. Una URL compartida no basta para acreditar todos los atributos.
4. **Límites ya cubiertos:** se conserva el retiro de prioridad global de O3, la diferencia de revisión global/local de AIT y la contradicción de agenda/dirección de Hackathons.team. No se presentan como problemas descubiertos por primera vez en esta corrección.
5. **Discrepancia temporal nueva:** el encabezado histórico de AIT etiqueta PDT en diciembre. `zoneinfo` para SF el 06/12/2025 devuelve PST (UTC−08). Se conserva la fecha local y no se transforma ese encabezado en un UTC verificado. No afecta a la fecha futura de O1.

Las fuentes de oportunidades y patrocinio se reabrieron; el registro por afirmación conserva métodos y límites de acceso. No se reprodujeron videos ni ejecutaron proyectos. La extracción automática y la experiencia de app siguen sin estar verificadas por DP-01.

Execution permanece verified para el alcance documental. No se certifica experiencia organizando de Vultr, resultados propios de Hackathons.team, factibilidad comercial ni UI. Los conteos de checks iniciales son históricos; las comprobaciones finales de esta revisión se registran a continuación.

**Comprobaciones finales de esta revisión:** 11 documentos y 83 enlaces locales válidos, incluidos sus anclajes; 51 registros de soporte (15 A, 28 B, 8 C), IDs únicos y correspondencia completa con R1–R15; siete criterios contrastados con la matriz. Cuatro fechas dentro de ventana, dos conversiones UTC/PDT y discrepancia histórica PST comprobadas con `zoneinfo`. `git diff --check` sin errores. Las huellas SHA-256 de 19 archivos ajenos de DP-02 se conservaron idénticas durante esta corrección; en el plan compartido solo se editó el resumen de DP-01. Los checks de enlaces/estructura no sustituyen la lectura de fuentes realizada arriba. No se modificó código de aplicación.
