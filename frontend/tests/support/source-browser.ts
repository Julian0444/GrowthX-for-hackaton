// DP-05 browser verification owns its copied Next instance and test session.
...
export async function startTestApp() {
  ...
  const productionDir=process.env.DP05_PRODUCTION_DIR;
  const temp=productionDir??mkdtempSync(path.join(tmpdir(),'growthx-dp05-e2e-'));
